...
export class ClapMeter {
  ...
  private findNeedleEntity(): Entity | null {
    ...
    if (needleEntities.length === 0) {
      ...
      console.error(`ClapMeter: Multiple children found with name starting with "Needle" (found ${needleEntities.length}). Expected exactly one.`)
      return null
    }
 ...
 */
  public resetClapMeter(newPlayer: string = "") {
    ...
    Transform.getMutable(this.arrow_entity).rotation = Quaternion.fromEulerDegrees(0, 0, this.currentNeedleRotation)
  }
